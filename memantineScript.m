...
resultspath = 'A:\OneDrive - Universidade Federal do Rio Grande do Sul\MATLAB\Gabi\20200810-artigo';
cd(resultspath) 
datapath = 'A:\OneDrive - Universidade Federal do Rio Grande do Sul\MATLAB\Gabi\Dados\';
Trechos; %load F;
...
for period = 2:3
    ...
    sd = errorbar(idx,:);
    for i = 4:-1:1
        plot(f,y(i,:),symbol{i},'linew',2,'Color',co(i,:));
        hold on;
%         plot(f(1:10:end),y(i,1:10:end),symbol{i},'MarkerFaceColor',co(i,:),'MarkerEdgeColor',co(i,:),'MarkerSize',5);
        patch([f fliplr(f)],[y(i,:)-sd(i,:) fliplr(y(i,:)+sd(i,:))],co(i,:),'FaceAlpha',0.2,'LineStyle','none')
    end
    ylabel('Power (mV²/Hz)')
    xlabel('Frequency (Hz)')
    switch period
...
for period = 1
    ...
    sd = errorbar(idx,:);
    for i = 1:2
        plot(f,y(i,:),symbol{2*i-1},'linew',2,'Color',co(2*i-1,:)); hold on;
        patch([f fliplr(f)],[y(i,:)-sd(i,:) fliplr(y(i,:)+sd(i,:))],co(2*i-1,:),'FaceAlpha',0.2,'LineStyle','none');
    end
    ylabel('Power (mV²/Hz)')
    xlabel('Frequency (Hz)')
    title('Basal')
